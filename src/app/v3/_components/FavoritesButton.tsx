...
export default function FavoritesButton({ className = "" }: { className?: string }) {
  ...
  return (
    <Link
      ...
      title="Избранное"
      className={`relative grid shrink-0 place-items-center rounded-full bg-white/[0.07] ring-1 ring-white/12 transition-colors hover:bg-white/[0.13] ${className}`}
    >
      <Heart
      ...
      />
      {showBadge && (
        <span className="absolute -right-0.5 -top-0.5 grid h-[18px] min-w-[18px] place-items-center rounded-full bg-emerald-500 px-1 text-[10px] font-extrabold leading-none text-[#06120c] tabular-nums">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </Link>
