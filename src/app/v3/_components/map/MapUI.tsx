...
/**
 * На каждом проданном участке стоит домик — сразу видно, что там уже
 * построились, и номер участка там уже никому не нужен.
 *
 * Рисунок свой, с нуля: контурный домик со скатной крышей и дверным проёмом,
 * слева деревце с округлой кроной на тонком стволе, оба на общей базовой
 * линии. Растровую иконку сюда не тащим ни в каком виде — 48px растягивается
 * на маркер 10–16px и мылит, а инлайновый путь остаётся чётким на любом зуме
 * и красится в серый нужного скина.
 *
 * Форма подчинена размеру, а не наоборот. Крыша симметричная, скаты доходят
 * до стен: любой «художественный» огрызок ската на 12px читается как
 * сломанный контур. Крона — круг: самая устойчивая к уменьшению фигура.
 * Мельче 13px дерево пропадает совсем, и остаётся один домик, растянутый на
 * всю иконку (см. WITH_TREE_PX) — на общем плане мобайла клетка участка даёт
 * глиф в 8px, и две фигуры в нём слипаются в кляксу.
 *
 * Силуэт под линиями залит цветом скина, а не прозрачен: клетка проданного
 * почти белая, но под домиком может оказаться дорога или подпись схемы, и без
 * заливки контур ломается. Заливка идёт отдельным путём без обводки — линии
 * рисуются поверх и остаются ровной толщины.
 */

/** Ниже этого экранного размера дерево не рисуем — только домик. */
const WITH_TREE_PX = 13;

/** Силуэт дома: крыша плюс коробка стен. Только заливка, без обводки. */
const HOUSE_BODY = "M16.6 6.3 10.5 12.2h1.7V19.3h8.8V12.2h1.7z";
/** Линии дома: скаты крыши, стены с базой, дверной проём. */
const HOUSE_LINES = [
  "M10.5 12.2 16.6 6.3l6.1 5.9",
  "M12.2 10.6V19.3H21V10.6",
  "M14.9 19.3v-4.8h3.4v4.8",
];
/** Крона: круг, самая устойчивая к уменьшению форма. */
const TREE_CROWN = { cx: 6.1, cy: 7.7, r: 4 } as const;
/** Ствол: одна вертикаль до общей с домом базовой линии. */
const TREE_TRUNK = "M6.1 11.7v7.6";
...
export function HouseGlyph({
  ...
  const tree = size >= WITH_TREE_PX;
  // Без дерева кадрируемся по одному домику: тот же экранный размер, но вся
  // площадь иконки уходит на него. Пересчёт идёт через viewBox, а не через
  // transform, — иначе масштаб потянул бы за собой и толщину линии.
  const span = tree ? 24 : 15;
  const box = tree ? "0 0 24 24" : "9.4 5.3 15 15";
  // Толщина линии считается от экранного размера, а не берётся из системы
  // координат иконки: домик живёт от 8 до 16px, и постоянная толщина viewBox
  // на нижнем краю вырождается в полпикселя и пропадает.
  const w =
    Math.round(Math.max(1.6, (0.9 / size) * 24) * (span / 24) * 100) / 100;
  const ink = muted ? "rgba(15,23,42,0.6)" : stroke;
  const body = muted ? "rgba(255,255,255,0.72)" : fill;
  ...
  return (
    ...
    >
      {tree && <circle {...TREE_CROWN} fill={body} />}
      <path d={HOUSE_BODY} fill={body} />
      <g
        fill="none"
        stroke={ink}
        strokeWidth={w}
      ...
      >
        {tree && <circle {...TREE_CROWN} />}
        {tree && <path d={TREE_TRUNK} />}
        {HOUSE_LINES.map((d) => (
          <path key={d} d={d} />
        ))}
      </g>
...
// ─────────────────────────────────────────────────────────────────
// Пульс свободного участка
// ─────────────────────────────────────────────────────────────────

/**
 * Живая точка свободного участка: «сюда смотри». Две вещи разом — ореол,
 * расходящийся кольцом от точки, и лёгкое дыхание самой точки.
 *
 * Почему двух мало по отдельности. Один ореол не работает там, где точка
 * стоит на заливке своего же тира: розовое по розовому не видно, а свободные
 * ровно так и лежат — цветная клетка и цветная точка в ней. Одно дыхание без
 * ореола на общем плане теряется: точка 11–18px, её ход в 12% — полтора
 * пикселя. Вместе читается на любой подложке.
 *
 * Почему CSS, а не перерисовка карты. Свободных у «Фаворита» 42, и каждый
 * кадр анимации через ymaps3 означал бы пересборку маркеров — карта на
 * телефоне этого не переживёт. Здесь же анимируются только transform и
 * opacity, всё уходит на композитор, а сама карта за время анимации не
 * перерисовывается ни разу.
 *
 * Период 2,6с: точка вздыхает и гаснет один раз в две с половиной секунды, а
 * не мигает. Ореол проявляется не в нуле, а на 12% — в нуле он ровно под
 * точкой и её белой обводкой, и вся видимая часть анимации уходила впустую.
 * Старт разведён по фазе от номера участка (детерминированно, без random) —
 * иначе все 42 ореола хлопают разом, и это читается как вспышка, а не как
 * живая карта.
 *
 * Классы держим здесь, а не в globals.css: правки по карте ограничены картой.
 */
export const PULSE_CLASS = "v3-plot-pulse";
export const BREATH_CLASS = "v3-plot-breath";
export const PULSE_PERIOD_S = 2.6;

export const PULSE_CSS = `
@keyframes v3-plot-pulse {
  0%   { transform: scale(1); opacity: 0; }
  12%  { transform: scale(1.4); opacity: 0.55; }
  100% { transform: scale(2.8); opacity: 0; }
}
@keyframes v3-plot-breath {
  0%, 100% { transform: scale(1); }
  46%      { transform: scale(1.14); }
}
.${PULSE_CLASS} {
  animation: v3-plot-pulse ${PULSE_PERIOD_S}s cubic-bezier(0.22, 1, 0.36, 1) infinite;
  pointer-events: none;
}
.${BREATH_CLASS} {
  animation: v3-plot-breath ${PULSE_PERIOD_S}s cubic-bezier(0.37, 0, 0.63, 1) infinite;
}
@media (prefers-reduced-motion: reduce) {
  .${PULSE_CLASS} { animation: none; opacity: 0; }
  .${BREATH_CLASS} { animation: none; }
}
`;

/** Фаза пульса по номеру участка: разброс без random и без Date. */
export function pulseDelay(seed: string): string {
  let h = 0;
  for (let i = 0; i < seed.length; i += 1) h = (h * 31 + seed.charCodeAt(i)) % 997;
  return `-${((h % 13) * (PULSE_PERIOD_S / 13)).toFixed(2)}s`;
}
            ...
            {hasSold && (
              <div className="flex items-center gap-2 px-1.5">
                {/* Серая клетка с домиком — ровно то, что видно на карте.
                    Клетка чуть крупнее прочих меток легенды: в 14px домик с
                    деревцем не разбираешь. */}
                <span
