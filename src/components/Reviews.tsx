...
export default function Reviews() {
  ...
  return (
    <section
      id="reviews"
      className="py-6 lg:py-10 bg-white dark:bg-transparent scroll-mt-16 relative overflow-hidden"
    >
      {/* Soft bg accent */}
      ...
      <div className="relative max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-6">
          ...
          <div className="inline-flex items-center gap-2.5 bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-500/10 dark:to-yellow-500/10 ring-1 ring-amber-200 dark:ring-amber-400/20 px-4 py-2 rounded-full shadow-sm dark:shadow-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
          {reviews.map((review) => {
            ...
            return (
              <div
                key={review.id}
                className="group relative bg-white dark:bg-white/5 rounded-2xl p-5 ring-1 ring-gray-200 dark:ring-white/10 hover:ring-emerald-300 dark:hover:ring-emerald-400/30 hover:shadow-lg dark:hover:shadow-none hover:-translate-y-0.5 transition-all"
              >
                {/* Big quote mark in corner */}
                <Quote
                  className="absolute top-3 right-3 w-6 h-6 text-emerald-100 dark:text-emerald-400/20 group-hover:text-emerald-200 dark:group-hover:text-emerald-400/40 transition-colors"
                  strokeWidth={2.5}
                />
        ...
        <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
          {[
            ...
            <div
              key={i}
              className="px-4 py-3 bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-500/10 dark:to-green-500/10 rounded-2xl ring-1 ring-emerald-200/70 dark:ring-emerald-400/20 text-center"
            >
              <div className="text-2xl font-black text-emerald-700 dark:text-emerald-300 leading-none tracking-tight tabular-nums">
                {stat.value}
              </div>
              <div className="text-[10px] font-bold uppercase tracking-wider text-emerald-900/70 dark:text-emerald-200/70 mt-1">
                {stat.label}
              </div>
            </div>
          ))}
