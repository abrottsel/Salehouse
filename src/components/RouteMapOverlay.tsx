...
/**
 * RouteMapOverlay — Yandex Maps 3.0 overlay rendered inside the iframe's
 * parent container (absolute inset-0). The iframe stays mounted
 * underneath and becomes visible again when `open = false`.
 *
 * Parent in /v4/village/[slug]/page.tsx has `relative overflow-hidden
 * rounded-2xl` — the overlay inherits those clips automatically.
 */
...
export default function RouteMapOverlay({
  ...
  // Load ymaps3 when the overlay first opens (cached by loader).
  useEffect(() => {
    if (!open) {
    ...
    const timeoutId = setTimeout(() => {
      if (cancelled) return;
      console.error("[RouteMapOverlay] ymaps3 load timed out after 15s");
      setBundle({
        kind: "blocked",
        error:
    ...
    loadYmaps3()
      ...
      .catch((err: Error) => {
        clearTimeout(timeoutId);
        if (cancelled) return;
        console.error("[RouteMapOverlay] ymaps3 load failed:", err);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        console.warn("[RouteMapOverlay] OSRM failed, using straight line:", err);
  ...
  // Reset fetched route when overlay closes so next open re-fetches fresh.
  useEffect(() => {
    if (!open) {
      setRoute(null);
      setRouteError(null);
  ...
  return (
    <div className="absolute inset-0 z-50 bg-gray-900 flex flex-col">
      ...
      <div className="flex items-center justify-between gap-3 px-4 py-3 bg-black/70 backdrop-blur-md ring-1 ring-white/10 shadow-lg">
